...
multilingual = pd.DataFrame(data['multilingual'])
combinations = pd.DataFrame(data['combinations'])
...
def get_combo(name):
    match = combinations[combinations['combo'] == name]
    return match['pct'].values[0] if len(match) > 0 else 0
```
...
fig.update_layout(
    height=400,
    margin=dict(t=20, r=20, b=80, l=60),
    plot_bgcolor="white",
    paper_bgcolor="white",
...
combos_reversed = combinations.iloc[::-1].reset_index(drop=True)
...
fig.add_trace(go.Bar(
    y=combos_reversed['combo'],
    x=combos_reversed['pct'],
    orientation='h',
    marker=dict(color=colors_gradient),
    text=[f"{p}%" for p in combos_reversed['pct']],
...
fig.update_layout(
    height=450,
    margin=dict(t=20, r=60, b=50, l=260),
    plot_bgcolor="white",
    paper_bgcolor="white",
